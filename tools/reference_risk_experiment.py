"""Exploratory covariance refinement on already-evaluated frozen RNA inputs."""
...
from src.reference_risk import fit_reference_ensembles
...
def run(args):
    ...
    if args.dataset == 'op3':
        ...
        for c in TARGET_CONTEXTS:
            ...
            for grouping in ('donor', 'plate'):
                factors, units = op3_factors(store, c, source, a, grouping)
                pred, rec = fit_reference_ensembles(source.means, a.indices, a.reference_c, a.reference_d,
                                                   factors, original['gram'])
                result[name].update({grouping+'__'+k: v for k, v in pred.items()})
                records[name][grouping] = {'reference_units': units, **rec}
        ...
        for c in CONTEXTS:
            ...
            pred, rec = fit_reference_ensembles(inputs['source'], inputs['anchor_indices'], inputs['anchor_c'],
                                               inputs['anchor_d'], factors, original['gram'])
            result[name] = {'plate__'+k: v for k, v in pred.items()}
            records[name] = {'reference_units': units, **rec}; sample_access[name] = store.access
    ...
    lock = {'scientific_status': 'exploratory_followup_not_fresh_confirmation', 'dataset': args.dataset,
            ...
            'code_sha256': {p: sha256_file(ROOT/p) for p in ['src/reference_risk.py','tools/reference_risk_experiment.py','src/rna_models.py']},
